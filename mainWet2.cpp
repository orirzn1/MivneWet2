...
vector<int> getRecordsStocks(int& size);
...
int main()
{
    ...
    while (cin >> op) {
        ...
        if (!op.compare("newMonth")) {
            // get params
            int size;
            vector<int> stocks_vec = getRecordsStocks(size);

            // call function
            StatusType status = test_obj->newMonth(&stocks_vec[0], size);
            print(op, status);
...
static const char* StatusTypeStr[] = {"SUCCESS", "ALLOCATION_ERROR", "INVALID_INPUT", "FAILURE", "ALREADY_EXISTS",
                                      "DOESNT_EXISTS"};
